...
# /home/pi/picamera-motion/flask_webserver.sh start

progpath="/home/pi/picamera-motion"
progname="flask_main.py"
...
if [ -z "$( pgrep -f $progpath/$progname )" ]; then
    ...
    echo "INFO    - Stopped. To Start $progpath/$progname execute command below"
    echo "INFO    - $0 start"
else
    progPID=$(pgrep -f $progpath/$progname)
    echo "STATUS  - $progpath/$progname is Running ..."
    echo "STATUS  - PID is $progPID"
    echo "INFO    - Started To Stop $progpath/$progname execute command below"
    echo "INFO    - $0 stop"
fi
echo "Done"
