...
def get_file_name(image_dir, image_name_prefix, current_count):
    ...
    if imageNumOn:
        ...
        right_now = datetime.datetime.now()
        #"%s/%s%04d%02d%02d-%02d%02d%02d.jpg"
...
#------------------------------------------------------------------------------
def take_day_image(image_path, image_path_dup):
    """
    ...
    with picamera.PiCamera() as camera:
        ...
        time.sleep(1)
        print("image capture")
        #output = np.empty(((imageWidth +2) * (imageHeight + 28) * 3,), dtype=np.uint8)
        #camera.capture(output, 'rgb')
        #output = output[:imageWidth, :take_day_image]
        #print("Making request")
        #response = requests.post("http://192.168.0.110:80/v1/vision/detection",files={"image":output}).json()
        #print("we got response")
        #print(response)
        
        camera.capture(image_path)
        camera.close()
        copyfile(image_path, image_path_dup)
        #requests.get("http://post-master.herokuapp.com/SendEmail") #Send to home security and let it decice
        #requests.get("http://sanhoo-home-security.appspot.com/IamAlive?id=3&alarmTriggered=Y")
...
def do_motion_detection():
    ...
    while True:
        ...
        file_name_dup = get_file_name(imagePath_dup, imageNamePrefix, current_count)
        take_day_image(file_name, file_name_dup)
        if imageNumOn:
            current_count += 1
